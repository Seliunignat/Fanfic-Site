...
export const TextEditPage = () => {
  ...
  useEffect(() => {
    ...
    const saveChangesHandler = async () => {
        try {
          const data_req = await request(`/api/text/update/${text._id}`, "POST", {text}, {Authorization: `Bearer ${auth.token}`})
          message(data_req.message);
          if(data_req.message === "Фанфик успешно отредактирован") history.goBack()
